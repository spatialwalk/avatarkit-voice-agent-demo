...
const HOST_SERVER_URL = process.env.NEXT_PUBLIC_HOST_SERVER_WS || 'ws://localhost:8765/ws/agent'
...
export default function ControlPanel({ activeAvatar, activeController, multiMode, avatarSlots, activeUid, onSlotSelect }: Props) {
  ...
  useEffect(() => { hostConnectedRef.current = hostConnected }, [hostConnected])
      ...
      if (!hostConnectedRef.current) {
        ...
        await new Promise<void>((resolve) => {
          const check = setInterval(() => {
            if (hostConnectedRef.current) { clearInterval(check); resolve() }
          }, 100)
          setTimeout(() => { clearInterval(check); resolve() }, 3000)
        })
      }
      if (!hostConnectedRef.current) return
      ...
      {hasAvatar && (
        <>
          <div className="btn-row" style={{ marginBottom: 8 }}>
            {!hostConnected ? (
              <button className="primary" disabled={hostConnecting} onClick={hostConnect}>
                {hostConnecting ? 'Connecting...' : 'Connect'}
              </button>
            ) : (
              <button className="secondary" onClick={hostDisconnect}>
                Disconnect
              </button>
            )}
          </div>
          ...
          <form className="host-text-form" onSubmit={(e) => { e.preventDefault(); sendTextQuery() }} style={{ marginTop: 8 }}>
            ...
            />
            <button type="submit" className="primary" disabled={!hostTextInput.trim() || !hasAvatar}>
              Send
            </button>
          </form>
